...
export async function GET(
  ...
  //set header
  pdf.setFontSize(24);
  pdf.text(data.invoiceName, 20, 20); // 20mm from left, 20mm from top
  ...
  pdf.text(data.invoiceItemsDescription, 20, 110);
  ...
  // pdf.output: converts the pdf to a buffer (low representation of binary data)
  // Buffer: converts arraybuffer to nodejs buffer object
  ...
  return new NextResponse(pdfBuffer, {
    headers: {
      ...
      "Content-Disposition": "inline", // inline to open in browser, attachment to download
    },
  });
}